...
static HAL_StatusTypeDef PCD_WriteEmptyTxFifo(PCD_HandleTypeDef *hpcd, uint32_t epnum)
{
  ...
  {
//		printf("\r\nepnum:%d DTXFSTS:0x%0x len32b:%d xfer_count:%d xfer_len:%d\r\n",epnum, (uint32_t*)(USBx_INEP(epnum)->DTXFSTS & USB_OTG_DTXFSTS_INEPTFSAV),len32b,ep->xfer_count,ep->xfer_len);	
    /* Write the FIFO */
    len = ep->xfer_len - ep->xfer_count;
